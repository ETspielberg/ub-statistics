...
import unidue.ub.statistics.userauth.UserDAO;
import unidue.ub.statistics.userauth.UserRole;
import unidue.ub.statistics.userauth.UserRoleDAO;
 ...
 * @version 1
 */
@WebServlet("/fachref/admin/userManagement")
public class UserManagementServlet extends FachRefServlet {
	...
	protected void doGet(MCRServletJob job) throws IOException, TransformerException, SAXException {
		...
		if (currentUser.hasRole("userAdmin")) {
        	...
        	for (String user : users) {
        		...
        		userElement.addContent(new Element("email").addContent(user));
