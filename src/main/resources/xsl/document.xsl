...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="yearsToAverage">
		<xsl:value-of
			select="/document/eventAnalysis/stockControlProperties/yearsToAverage" />
	</xsl:variable>
		...
		<html>
			...
			<body>
				...
				<div class="jumbotron">
					<div class="container">
						<h1>Analyse</h1>
						<p>Zusammensetzung und Ausleihanalyse</p>
					</div>
				</div>
	...
	<xsl:template match="document">
		...
		<!-- <div class="col-sm-9 col-md-10 col-md-offset-1 main"> <xsl:call-template 
			name="deletion" /> </div> -->
	</xsl:template>

	<xsl:template name="form">
		<h2 class="sub-header">Zeitraum ändern</h2>
		<form action="analytics" method="get">
			<label for="yearsToAverage"
				style="display: inline-block; width: 240px; text-align: right">
				Zeitraum in Jahren:
			</label>
			<input type="text" size="3" value="{$yearsToAverage}" name="yearsToAverage"
				id="yearsToAverage" />
			<xsl:for-each select="/document/documentsAnalyzed/documentAnalyzed">
		...
		<p>
			Der angegebene Zeitraum wird zur Berechnung der durchschnittlichen und
			maximalen Ausleihe herangezogen. Ausleihen in Handapparate werden
			nicht als Ausleihe gezählt,
	...
	<xsl:template match="eventAnalysis">
		<xsl:choose>
			<xsl:when test="analysis">
				<h2 class="sub-header"> Ausleih-Analyse </h2>
	...
	<xsl:template name="plot">
		<div>
			...
			<div class="well" id="relativeLoanDescription" style="display:none">
				Die Abbildung zeigt die relative Ausleihe der verschiedenen
				Benutzergruppen, bezogen auf den jeweils aktuellen Bestand.
				Die obere horizontale Linie gibt die maximale relative Ausleihe im
				gewählten Zeitraum an, die untere die durchschnittliche relative
				Ausleihe.
			...
			<div class="well" id="absLoanDescription" style="display:none">
				Die Abbildung zeigt die absoluten Ausleihzahlen und Bestände der
				einzelnen Auflagen additiv übereinander dargestellt.
				Die horizontale Linie gibt die maximale absolute Ausleihe im gewählten
				Zeitraum an.
			</div>
		</div>
	</xsl:template>
		...
		<div id="parent" class="container">
			<script type="text/javascript" charset="utf-8">
				...
				<xsl:for-each select="//eventAnalysis/analysis">
					yearsToAverage =
					<xsl:value-of select="$yearsToAverage" />
					;
					lastStock =
					<xsl:value-of select="lastStock/." />
					;
					...
					;
					maxValue =
					<xsl:value-of select="maxRelativeLoan/." />
					;
					deletionProposal =
					<xsl:value-of select="proposedDeletion/." />
					;
				...
				getTimelineAllFilename =
				"../series?type=&amp;"+listDocumentNumbersString;
				...
				*/
				...
				document.getElementById('relativeLoanDescription').style.display =
				'block';
				...
				var yAxis =
				d3.svg.axis().scale(y).orient("left").tickFormat(d3.format(".0%"));;
				...
				*/
				...
				document.getElementById('absLoanDescription').style.display =
				'block';
				...
				//declare the x- and y-axis span
				x.domain([d3.min(stockEditions, function(d) {return d3.min(d.values, function (d) { return
				d.time;});}),d3.max(stockEditions, function(d) { return
				d3.max(d.values, function (d) { return d.time; }); })]);
				y.domain([0,d3.max(stockEditions, function(d) { return
				d3.max(d.values, function (d) { return d.y + d.y0; }); })]);
				...
				d3.select("#yearsToAverageNew").property("value",
				yearsToAverageNew);
