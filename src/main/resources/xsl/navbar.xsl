		...
		<nav class="navbar navbar-inverse navbar-fixed-top">
			<div class="container-fluid">
				...
				<div class="navbar-header">
					...
					<a class="navbar-brand" href="{$WebApplicationBaseURL}fachref/start">FachRef-Assistent</a>
					<xsl:if test="module">
					...
					<xsl:when test="loggedInAs">
						<div id="navbar" class="navbar-collapse collapse">
							<ul class="nav navbar-nav navbar-right">
								<li>
									<xsl:if test="module">
										<xsl:if test="module/. = 'Info' ">
											<xsl:attribute name="class">active</xsl:attribute>
										</xsl:if>
									</xsl:if>
									<a href="{$WebApplicationBaseURL}help">Info</a>
								</li>
								<li>
									<xsl:if test="module">
										<xsl:if test="module/. = 'Protokoll'">
											<xsl:attribute name="class">active</xsl:attribute>
										</xsl:if>
									</xsl:if>
								...
								<li>
									<xsl:if test="module">
										<xsl:if test="module/. = 'Profile'">
											<xsl:attribute name="class">active</xsl:attribute>
										</xsl:if>
									</xsl:if>
									<a href="{$WebApplicationBaseURL}fachref/profile">Profile</a>
								</li>
								<li>
									<xsl:if test="module">
										<xsl:if test="module/. = 'Hitlisten'">
											<xsl:attribute name="class">active</xsl:attribute>
										</xsl:if>
									</xsl:if>
									<a href="{$WebApplicationBaseURL}fachref/hitlists">Hitlisten</a>
								</li>
								<li>
									<xsl:if test="module">
										<xsl:if test="module/. = 'e-Journals'">
											<xsl:attribute name="class">active</xsl:attribute>
										</xsl:if>
									</xsl:if>
									<a href="{$WebApplicationBaseURL}fachref/journals">e-Journals</a>
								</li>
								<li>
									<xsl:if test="module">
										<xsl:if test="module/. = 'Bestand'">
											<xsl:attribute name="class">active</xsl:attribute>
										</xsl:if>
									</xsl:if>
									<a
										href="{$WebApplicationBaseURL}fachref/stock/usage?collections=&amp;materials=BOOK">Bestand</a>
								</li>
								<li class="dropdown">
									...
									<ul class="dropdown-menu" id="login-dp">
										...
										<li>
											<a href="{$WebApplicationBaseURL}fachref/settings/User_Form.xed">Einstellungen</a>
										</li>
										<li>
											<a href="{$WebApplicationBaseURL}fachref/settings/passwordChange">Passwort ändern</a>
										</li>
										<xsl:if test="admin">
											<li>
												...
												<div class="col-md-12">
													...
													<form class="form" role="form" method="post" action="{$WebApplicationBaseURL}userLogging"
														...
														<div class="form-group">
															<label class="sr-only" for="exampleInputEmail2">Benutzername</label>
															<input type="email" class="form-control" id="exampleInputEmail2"
																placeholder="Kennung" name="email" required="true"
																autofocus="true" />
														</div>
														<div class="form-group">
